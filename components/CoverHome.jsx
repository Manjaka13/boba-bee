...
const CoverHome = () => {
	...
	useEffect(() => {
		const id = setInterval(() => {
			setCurrentMilktea(prevState => prevState + 1 >= milkteaList.length ? 0 : prevState + 1);
		}, 2000);
	...
	const mappedMilktea = milkteaList.map((milktea, key) => (
		<img
			key={ key }
			className={ `milktea__item ${key === currentMilktea ? "milktea__item--shown" : ""} w-100 h-100 tr-200` }
			src={ milktea.image }
			alt={ milktea.name }
		/>
	));
				...
				<div className="cover-home__overlay w-100 h-100">
					<div className="w-100 h-70">
						<div className="container h-100 o-h f-c-ce-ce">
							<h1 className="cover-home__title fs-400 white tx-c mg-b-40">Taste like <span className="t">honey</span> pearl</h1>
							<p className="tx-c f-r-ce-ce">
								...
								<img className="cover-home__honeycomb" src="/images/honeycomb.png" alt="Honeyomb" />
